...
covid = rbind(
  fread(here("data/2021-22/0.txt"),nrows=1) %>% select(cause=`Underlying Cause of death`,deaths=Deaths) %>% mutate(agegroup="[0,1)"),
  fread(here("data/2021-22/1to4.txt"),nrows=1) %>% select(cause=`Underlying Cause of death`,deaths=Deaths) %>% mutate(agegroup="[1,5)"),
  fread(here("data/2021-22/5to9.txt"),nrows=1) %>% select(cause=`Underlying Cause of death`,deaths=Deaths) %>% mutate(agegroup="[5,10)"),
  fread(here("data/2021-22/10to14.txt"),nrows=1) %>% select(cause=`Underlying Cause of death`,deaths=Deaths) %>% mutate(agegroup="[10,15)"),
  fread(here("data/2021-22/15to19.txt"),nrows=1) %>% select(cause=`Underlying Cause of death`,deaths=Deaths) %>% mutate(agegroup="[15,20)"))
...
wonder$rate = round(wonder$deaths / wonder$pop * 100000,1)
...
g = g + scale_y_continuous(expand=c(0,0),limits=c(0,4.5))
...
covid0_19 = fread(here("data/2021-22/0to19.txt"),nrows=1) %>%
...
d$rate = round(d$deaths / d$pop * 100000  ,1)
